...
class SpineMLIRCodeGenerator(ast.NodeVisitor):
    ...
    def _gen_vload(self, node: ast.Call, hint: str) -> tuple[str, str]:
        ...
        if valid_node is not None or valid_ssa_active is not None:
            ...
            # 要不要 pad 由 codegen 编译期决定(此 Python 分支), 不往 IR 塞运行期 scf.if:
            # kernel 用 strip-mine 把 K 循环分裂成主循环(满 tile, 不设 vconfig 收窄 →
            # _active_valid=None → 走下面的快路)与尾循环(设 vconfig(K-ki) → 进本分支走 fill-0)。
            # 尾循环的 scf.for 天然跑 0 次(K%VL==0)或 1 次, 用迭代次数代替分支。整除 shape 全走
            # 主循环快路, 零 fill-0 零分支 → 消除 512×512 曾 +60% 的回归。
