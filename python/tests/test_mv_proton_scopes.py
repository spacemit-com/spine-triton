...
def mv(B: In["memref<*xf16, #ptr.generic_space>"], A: In["memref<*xf16, #ptr.generic_space>"],
       col: In["index"], M: In["index"], nk: In["index"], C: InOut["memref<*xf32, #ptr.generic_space>"]):
    sr_mod.proton_mark("alloc", True)
    b0=sr_mod.alloc_tcm_2d(32, 64, "f16"); b1=sr_mod.alloc_tcm_2d(32, 64, "f16")
    b2=sr_mod.alloc_tcm_2d(32, 64, "f16"); b3=sr_mod.alloc_tcm_2d(32, 64, "f16")
    sr_mod.proton_mark("alloc", False)
    sr_mod.proton_mark("splat", True)
    a0=sr_mod.splat_2d(0.0, 1, 64, "f32"); a1=sr_mod.splat_2d(0.0, 1, 64, "f32")
    a2=sr_mod.splat_2d(0.0, 1, 64, "f32"); a3=sr_mod.splat_2d(0.0, 1, 64, "f32")
    sr_mod.proton_mark("splat", False)
    for kb in sr_mod.range(nk):
        koff=kb*32; lhs=sr_mod.view_2d(B, 1, 32, "f16", koff)
        ...
        r0=sr_mod.load_2d(b0, 32, 64, "f16"); r1=sr_mod.load_2d(b1, 32, 64, "f16")
        r2=sr_mod.load_2d(b2, 32, 64, "f16"); r3=sr_mod.load_2d(b3, 32, 64, "f16")
        sr_mod.proton_mark("load", False)
        sr_mod.proton_mark("macc", True)
        a0=sr_mod.batch_macc(lhs, r0, a0); a1=sr_mod.batch_macc(lhs, r1, a1)
        a2=sr_mod.batch_macc(lhs, r2, a2); a3=sr_mod.batch_macc(lhs, r3, a3)
        sr_mod.proton_mark("macc", False)
    sr_mod.proton_mark("store", True)
    sr_mod.store_2d_at(C, col, 1, 64, a0); sr_mod.store_2d_at(C, col+64, 1, 64, a1)
    sr_mod.store_2d_at(C, col+128, 1, 64, a2); sr_mod.store_2d_at(C, col+192, 1, 64, a3)
